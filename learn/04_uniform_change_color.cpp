...
g，准备直接抄标准代码。
05_xxx.cpp 可以运行之后 来这里 修复了，现在可以了， 是因为 glBindVertexArray(VAO) 和 glVertexAttribPointer 的语句顺序问题， 必须前面的先写。不然的话，不会显示图形。 我以为 glBindVertexArray(VAO) 只是 怎么解析，但是看下来，这条语句 已经拿到 VAO的地址/数据 了。  或者说 是 glEnableVertexAttribArray 的时候拿到 VAO的数据。。 确实，看名字， 一个是 绑定 VertexArray，一个是 VertexAttribPointer。 都是 VertexArray的， 所以 后者 依赖了 前者。。  对， 数据 和 操作分离， 所以 VertexAttri 没有指定 操作的数据， 这个数据 是由 BindVertexArray 来指定的。  所以 必须 Bind 在前。
和 glEnableXXX 无关，  只是 Bind 和 Pointer 之间的关系
...
float vertices[] = {
    0.5f, 0.5f, 0.0f,.0f,.0f,1.f,   // 右上角
    0.5f, -0.5f, 0.0f,0.f,1.f,0.f,  // 右下角
    -0.5f, -0.5f, 0.0f,0.f,0.f,1.f, // 左下角
    -0.5f, 0.5f, 0.0f,1.f,0.f,0.f   // 左上角
};
...
int main() {
    ...
    // glBindBuffer(GL_ARRAY_BUFFER, VBO);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    // glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);   // ..不能放这里。
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        // float greenVal = (sin(tm) / 2.0f) + 0.5f;
        // int vtxColLoc = glGetUniformLocation(shaderProgram, "myCol");

        // glUniform4f(vtxColLoc, 0.0f, greenVal, 0.f, 1.f);
